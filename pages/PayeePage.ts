...
export class PayeePage {
  ...
  private get successMessage(): Locator {
    return this.page
      .locator('text=/success|successfully|submitted|completed|request created|transaction submitted|payee added|beneficiary added/i')
      .first();
  }
  ...
  private get validationMessage(): Locator {
    return this.page
      .locator('text=/required|mandatory|invalid|please enter|please provide|enter valid/i')
      .first();
  }
  ...
  async validateSuccessfulSubmission() {
    ...
    throw new Error(
      'Payee creation success indicator was not found after final confirm.\nScreenshot saved as payee-success-validation-failed.png'
    );
  }
  ...
  async createValidPayeeAndVerify(accountNumber: string, nickname: string, email: string) {
    if (!nickname.trim()) {
      throw new Error('Valid payee creation received blank nickname.\nPlease check the valid test data.');
    }
